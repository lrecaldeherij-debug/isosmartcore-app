-- iso_migration_v14_seed_organization.sql
-- Plantillas iniciales para una organización nueva.
-- El owner llama a `seed_organization(org_id)` (vía RPC) desde la guía de
-- implementación. La función es idempotente: si el módulo ya tiene datos, no
-- inserta nada (no pisa el trabajo del usuario).
--
-- Cubre los módulos "documentales" (no datos operativos vivos como personal o
-- producción). El usuario edita las plantillas para adaptarlas a su realidad.
...
BEGIN
  ...
  IF v_count = 0 THEN
    INSERT INTO risk_matrix (org_id, process_name, risk_description, prob_initial, impact_initial, control_measure, responsible, status) VALUES
...
-- Permitir invocar la función desde el cliente (authenticated)
