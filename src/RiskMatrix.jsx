...
export default function RiskForm({ alGuardar, alCancelar }) {
  const [form, setForm] = useState({
    risk_description: '',
    probability_initial: 1, // Escala 1-5
    impact_initial: 1      // Escala 1-5
  })
  ...
  const handleSubmit = async (e) => {
    ...
    // Calcular la puntuación inicial (Nivel de Riesgo)
    const score = form.probability_initial * form.impact_initial;

    const { error } = await supabase.from('risk_matrix').insert([
      { ...form, score_initial: score }
    ])
