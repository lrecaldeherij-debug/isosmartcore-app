-- =============================================================================
-- v20 — Siembra granular por módulo
--
-- seed_organization (v14/v19) siembra TODOS los módulos vacíos de una.
-- Esta función permite sembrar UN módulo específico, llamada desde el banner
-- "Empezar con plantilla" dentro de cada pantalla individual (FODA, Riesgos, etc.).
--
-- Reglas:
--   * Solo el owner puede ejecutar.
--   * Si el módulo ya tiene datos, no hace nada (idempotente).
--   * Devuelve JSONB { "<module_key>": <cantidad_insertada> }
--   * module_key válidos: 'context', 'stakeholders', 'scope', 'processes',
--     'policy', 'roles', 'risks', 'objectives', 'documents'
-- =============================================================================

CREATE OR REPLACE FUNCTION seed_module(target_org_id UUID, module_key TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF module_key = 'context' THEN
    ...
    IF v_count = 0 THEN
      ...
      v_inserted := jsonb_set(v_inserted, '{context}', '8');
    END IF;

  ELSIF module_key = 'stakeholders' THEN
  ...
  ELSIF module_key = 'scope' THEN
    ...
    IF v_count = 0 THEN
      ...
      v_inserted := jsonb_set(v_inserted, '{scope}', '1');
    END IF;

  ELSIF module_key = 'processes' THEN
  ...
  ELSIF module_key = 'roles' THEN
    ...
    IF v_count = 0 THEN
      ...
      v_inserted := jsonb_set(v_inserted, '{roles}', '3');
    END IF;

  ELSIF module_key = 'risks' THEN
    ...
    IF v_count = 0 THEN
      ...
      v_inserted := jsonb_set(v_inserted, '{risks}', '5');
    END IF;

  ELSIF module_key = 'objectives' THEN
    ...
    IF v_count = 0 THEN
      ...
      (target_org_id, 'Reducir las no conformidades internas', 'Cantidad máxima de NC abiertas al cierre de mes', 5, 0, 'NC/mes', 'Mensual', 'Responsable de Calidad'),
      ...
      v_inserted := jsonb_set(v_inserted, '{objectives}', '4');
    END IF;

  ELSIF module_key = 'documents' THEN
    ...
    IF v_count = 0 THEN
      ...
      v_inserted := jsonb_set(v_inserted, '{documents}', '4');
    END IF;

  ELSE
    RAISE EXCEPTION 'Módulo desconocido: %', module_key;
  END IF;
...
GRANT EXECUTE ON FUNCTION seed_module(UUID, TEXT) TO authenticated;
